...
BEGIN
    ...
    SELECT
        ...
        ISNULL((
            SELECT
                SUM(CASE WHEN UPPER(LTRIM(RTRIM(v2.VType))) IN ('R', 'RECEIPT') THEN ISNULL(v2.Amount, 0) ELSE 0 END)
                - SUM(CASE WHEN UPPER(LTRIM(RTRIM(v2.VType))) IN ('P', 'PAYMENT') THEN ISNULL(v2.Amount, 0) ELSE 0 END)
