-- ============================================================
-- ACVoucher audit fields only
-- Add: CreatedDate, ModifiedDate, CreatedUserID, ModifiedUserID
-- Insert → Created*; Update → Modified*
-- Do NOT touch LedgerHeadDetail / LedgerHeadNarrationDetail (on hold)
-- ============================================================
SET NOCOUNT ON;
GO

IF COL_LENGTH('dbo.ACVoucher', 'CreatedDate') IS NULL
BEGIN
    ALTER TABLE dbo.ACVoucher ADD CreatedDate DATETIME NULL;
    PRINT 'Added ACVoucher.CreatedDate';
END
GO

IF COL_LENGTH('dbo.ACVoucher', 'ModifiedDate') IS NULL
BEGIN
    ALTER TABLE dbo.ACVoucher ADD ModifiedDate DATETIME NULL;
    PRINT 'Added ACVoucher.ModifiedDate';
END
GO

IF COL_LENGTH('dbo.ACVoucher', 'CreatedUserID') IS NULL
BEGIN
    ALTER TABLE dbo.ACVoucher ADD CreatedUserID BIGINT NULL;
    PRINT 'Added ACVoucher.CreatedUserID';
END
GO

IF COL_LENGTH('dbo.ACVoucher', 'ModifiedUserID') IS NULL
BEGIN
    ALTER TABLE dbo.ACVoucher ADD ModifiedUserID BIGINT NULL;
    PRINT 'Added ACVoucher.ModifiedUserID';
END
GO
...
BEGIN
    ...
    BEGIN
        -- Allocate next VCode under lock to avoid duplicates
...
PRINT '066_ACVoucher_Audit_Columns applied.';
GO
